...
const HomeJobsSection: React.FC<HomeJobsSectionProps> = ({ jobsData }) => {
  return (
    <section className="w-full flex flex-col gap-2 items-start justify-start">
      ...
      <span className="w-full flex lg:flex-row md:flex-row flex-col justify-between gap-4">
        ...
        <Link
          prefetch={true}
          href="/jobs"
          className="bg-primary p-2 rounded-md text-center text-h5 hover:opacity-80 opacity-100 text-white self-end"
        >
          See All Jobs
        </Link>
      </span>
      <JobsSection jobs={jobsData?.results?.slice(0, 3)} />
    </section>
  );
};
