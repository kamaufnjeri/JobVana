...
// interface for ApplicationModalProps
interface ApplicationModalProps {
  application: ApplicationProps; // data of the application in modal
  fetchApplications: (filters?: JobFilterProps) => void; // function to fetch applications
  closeModal: () => void; // function to close the modal
}

// type for update key fields that can be used
type UpdateProps = "availability" | "linkedin_url" | "cover_letter" | "resume";
...
const SingleJobApplicationModal: React.FC<ApplicationModalProps> = ({
  ...
  // handles which field is being updated at a time
  ...
  const [displayApplication, setDisplayApplication] =
    useState<ApplicationProps>(application); // sets application data to be displayed
  const [deleteApplication, setDeleteApplication] = useState<boolean>(false); // state to allow delete of application
  ...
  // function to allow editing of a specific field
  ...
  // cancel's editing of a specific field
  ...
  // function to handle change in resume pdf or new pdf chosen
  const handleResumeChange = (resume: File | string) => {
    setDisplayApplication((prev) => ({ ...prev, resume: resume }));
  };

  // handles change in input or textarea fields
  const handleChange = (
        ...
        <div className="flex flex-col w-full gap-6 p-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold">
              {displayApplication?.job_details.title}
            </h2>
            {displayApplication.created_at && (
              <p className="text-gray-500 text-sm">
                Date Applied: {formatDate(displayApplication.created_at)}
              </p>
            )}
          </div>
          ...
          <div
            className={`flex flex-col ${
              deleteApplication ? "lg:flex-col" : "lg:flex-row"
              ...
              <div className="flex flex-col gap-2 justify-center items-center text-center">
                ...
                <div className="flex flex-row gap-2">
                  ...
                  <Button
                    ...
                    styles="bg-gray-700 rounded-md text-white h-10 px-4"
                  />
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                {isDisabled.cover_letter ? (
                  <Button
                    ...
                    styles="bg-primary rounded-md text-white h-10 px-4"
                  />
                ) : (
                  <>
                    <Button
                      name="Save"
                      loading={loading}
                      onClick={() => handleApplicationUpdate("cover_letter")}
                      styles="bg-primary rounded-md text-white h-10 px-4"
                    />
                    <Button
                      name="Cancel"
            ...
            <span className="flex flex-col gap-2">
              ...
              {isDisabled.resume ? (
                ...
                <PDFInputField
                  resume={displayApplication.resume}
                ...
                {isDisabled.resume ? (
                  ...
                  <>
                    <Button
                      name="Save"
                      loading={loading}
                      onClick={() => handleApplicationUpdate("resume")}
                      styles="bg-primary rounded-md text-white h-10 px-4"
                    />
                    <Button
                      name="Cancel"
            ...
            <span className="flex flex-col gap-2">
              ...
              <Select
                ...
                value={
                  displayApplication.availability
                    ? AVAILABILITY_OPTIONS.find(
                        (option) =>
                          option.value === displayApplication.availability
                      )
                    : null
                }
                ...
                {isDisabled.availability ? (
                  ...
                  <>
                    <Button
                      name="Save"
                      loading={loading}
                      onClick={() => handleApplicationUpdate("availability")}
                      styles="bg-primary rounded-md text-white h-10 px-4"
                    />
                    <Button
                      name="Cancel"
            ...
            <span className="flex flex-col gap-2">
              <label className="text-h6 font-medium">
                ...
                {isDisabled.linkedin_url ? (
                  ...
                  <>
                    <Button
                      name="Save"
                      loading={loading}
                      onClick={() => handleApplicationUpdate("linkedin_url")}
                      styles="bg-primary rounded-md text-white h-10 px-4"
                    />
                    <Button
                      name="Cancel"
