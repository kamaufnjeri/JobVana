...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="w-[60px] flex flex-col gap-5 border-borderColor border-2 p-4 h-full rounded-lg shadow-lg">
      <Link href={user.role === 'applicant' ? '/dashboard/applicant' : '/dashboard/employer'} className='group relative flex justify-center'>
     ...
     {(user  && user?.role === 'applicant') && <Link href='/dashboard/saved-jobs' className='group relative flex justify-center'>
      ...
      <Link href='/dashboard/profile' className='group relative flex justify-center'>
