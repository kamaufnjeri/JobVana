...
interface ApplicationReceivedModalProps {
  application: ApplicationProps; // application data to view on modala
  fetchApplications: (filters?: JobFilterProps) => void; // function to fetch applications
  closeModal: () => void; // closes modal
}

const SingleApplicationReceivedModal: React.FC<
  ApplicationReceivedModalProps
> = ({ application, fetchApplications, closeModal }) => {
  const [isStatusDisabled, setIsStatusDisabled] = useState(true); // allow or disallow editing status
  const [loading, setLoading] = useState(false);
  const [displayApplication, setDisplayApplication] =
    useState<ApplicationProps>(application);

  // handles updating status of an application by employer
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        // on status update success update the application data
        setDisplayApplication(response.data.application);
        toast.success(response.data.message);
        fetchApplications();
      } else {
        toast.error(
          response.data.error || "Failed to update application status"
        );
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-background rounded-lg p-6 w-full max-w-2xl flex flex-col gap-4 relative h-auto max-h-[90vh] overflow-y-auto shadow-lg">
        {/* Close Button */}
        <FaTimes
          className="cursor-pointer text-2xl hover:text-primary absolute top-4 right-4"
          onClick={closeModal}
        />
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold">
            {displayApplication?.applicant_details.name}
          </h2>
        ...
        <div>
          ...
          <Select
            ...
            value={APPLICATIONS_STATUS_OPTIONS.find(
              (option) => option.value === displayApplication.status
            )}
          ...
          <div className="flex justify-end space-x-2 mt-3">
            {isStatusDisabled ? (
              <Button
                name="Change"
                onClick={() => setIsStatusDisabled(false)}
                styles="bg-primary rounded-md text-white h-10 p-2 self-end"
              />
            ) : (
              <>
                <Button
                ...
                <Button
                  ...
                  styles="bg-gray-800 rounded-md text-white h-10 p-2 self-center"
                />
              </>
            )}
          </div>
        ...
        <div>
          ...
          {displayApplication.linkedin_url && (
            <Link
              href={displayApplication.linkedin_url}
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button
                name="View Profile"
                styles="bg-blue-600 text-white flex items-center space-x-2"
              >
                <FaArrowRight />
              </Button>
            </Link>
